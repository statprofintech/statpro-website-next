...
// Interactive sub-components are isolated into three client files:
//   ProductPageHeroClient   → useState + useEffect (auto-advancing slider)
//   ProductPageEligibilityClient → useState + useMemo (eligibility calculator)
//   ProductPageFaqClient    → useState (accordion)
// This makes the H1 paint from the first HTML byte → fixes LCP on all product pages.

import Link from "next/link";
import {
  ArrowRight, ArrowUpRight, Check, X, Sparkles, FileText,
  ShieldCheck, Activity, Wallet, Repeat, Building,
  TrendingDown, RefreshCw, LineChart, Receipt, Calculator, MapPin, Banknote,
} from "lucide-react";
import ProductPageHeroClient from "@/components/ProductPageHeroClient";
import ProductPageEligibilityClient from "@/components/ProductPageEligibilityClient";
import ProductPageFaqClient from "@/components/ProductPageFaqClient";
...
export default function ProductPageLayout({ config }) {
  return (
    <div>
      <ProductPageHeroClient c={config} />
      <ValueStrip c={config} />
      <ProductPageEligibilityClient c={config} />
      <RateTable c={config} />
...
// (Faq is now in ProductPageFaqClient.jsx)
